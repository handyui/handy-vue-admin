          ...
          <template v-for="(action, index) in actions">
            ...
            <template v-if="action.type == 'button'">
              <!-- <a-button v-permission="action.permission"
                        ...
                        :key="index"> -->
              <a-button v-bind="{...buttonProps,...action.props}" @click="actionEvent(slotProps.record, action.func)"
                  :key="index">
                {{ action.text }}
              </a-button>
            </template>
            ...
            <template v-if="action.type ==  'text'">
              <a v-bind="{ ...action.props}" @click="actionEvent(slotProps.record, action.func)"
                  :key="index">
                {{ action.text }}
              </a>
            </template>
            ...
            <template v-if="action.type == 'popconfirm'">
              <a-popconfirm :key="index" placement="leftTop" @confirm="actionEvent(slotProps.record, action.func)">
                ...
                <a  v-bind="{...action.props}">{{ action.text }}</a>
              </a-popconfirm>
            </template>
          </template>
          <!--        对表格的操作动作end-->
...
<script lang="ts">
import {defineComponent, reactive, PropType, toRefs, onMounted, toRaw} from 'vue'
import {Card, Select, Table, Popconfirm, message, Result} from 'ant-design-vue'
...
export default defineComponent({
  ...
  setup(props: Props, {attrs, slots}) {
    ...
    const refreshTableData = async (param: PagePrams = {
      current: state.pageOption.current ,
      size: state.pageOption.pageSize ,
      ...props.pageOption
    }) => {
      state.tableLoading = true
      const result = await props.getListFunc({current:1, size:10, ...props.queryParams, ...param}).finally(() => state.tableLoading = false)
      const {records, current, size, total, pages} = result.result
      Object.assign(state.pageOption, {current, pages, size, total})

      // 因为没有传回children字段，要自行添加
      if(props.getListFunc.name == 'queryCatalogList'){
    ...
    const paginationChange = (pagination) => {
      ...
      refreshTableData({size: pagination.pageSize, current: pagination.current, ...props.pageOption})
    }
